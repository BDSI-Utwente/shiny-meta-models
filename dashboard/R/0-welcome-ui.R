...
welcomeUI <- tabItem(
  ...
  box(
    ...
    p("The Probabilistic Analysis Check dashBOARD (PACBOARD) is an interactive tool to systematically explore and validate the inputs and outputs of a probabilistic analysis obtained from a health economic model.", 
      br(),
      "To use PACBOARD, you need a dataset (or dataframe) containing both inputs and (intermediate) outputs of a probabilistic analysis. The first row of the dataset should represent the input and output names."),
    p("PACBOARD is composed of 6 different tabs:", 
      br(),
      "1. The", strong("Prepare data-tab"), ": upload your data and group variables in 'costs', '(dis)utilities', 'probabilities', and 'relative effectiveness'. In this tab, the first quick checks are performed on your inputs and outputs You can also define whether you want to calculate the incremental costs and effects of your intervention and the (incremental) net benefits if the dataset does not contain it already.",
      br(),
